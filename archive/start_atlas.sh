...
# MCP Proxy режим видалено – завжди direct mode (апле скріпт + playwright)
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "Proxy режим видалено – використовується тільки direct mode"
            exit 0
            ;;
    esac
done
...
info "� Режим: Direct Services (AppleScript + Playwright)"
...
## MCP Proxy вилучено – крок пропущено
...
info "   🔗 MCP Proxy:         Видалено (direct mode only)"
...
echo ""
info "💡 MCP Proxy прибрано з системи"
