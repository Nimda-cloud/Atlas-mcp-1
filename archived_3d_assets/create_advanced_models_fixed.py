#!/usr/bin/env python3
"""
Створення додаткових простих 3D моделей голов
"""
...
def create_alien_head():
    ...
    # Додаємо великі очі
    left_eye = trimesh.creation.icosphere(radius=0.3)
    left_eye.apply_translation([-0.4, 0.2, 0.8])
    
    right_eye = trimesh.creation.icosphere(radius=0.3)
    right_eye.apply_translation([0.4, 0.2, 0.8])
    
    # Об'єднуємо
    alien = trimesh.util.concatenate([head, left_eye, right_eye])
...
def create_dragon_head():
    """Створює голову дракона"""
    # Основна форма - витягнута
    head = trimesh.creation.icosphere(subdivisions=2, radius=1.0)
    
    vertices = head.vertices.copy()
    
    # Витягуємо морду
    snout_mask = vertices[:, 2] > 0.3
    vertices[snout_mask, 2] *= 1.8
    
    # Звужуємо морду
    vertices[snout_mask, 0] *= 0.7
    vertices[snout_mask, 1] *= 0.8
    
    head.vertices = vertices
    
    # Роги дракона
    horn1 = trimesh.creation.cylinder(radius=0.15, height=0.8)
    horn1.apply_transform(trimesh.transformations.rotation_matrix(np.pi/4, [1, 0, 0]))
    horn1.apply_translation([-0.3, 0.6, -0.2])
    
    horn2 = trimesh.creation.cylinder(radius=0.15, height=0.8)
    horn2.apply_transform(trimesh.transformations.rotation_matrix(np.pi/4, [1, 0, 0]))
    horn2.apply_translation([0.3, 0.6, -0.2])
    
    # Центральний рог
    center_horn = trimesh.creation.cylinder(radius=0.12, height=1.0)
    center_horn.apply_transform(trimesh.transformations.rotation_matrix(np.pi/3, [1, 0, 0]))
    center_horn.apply_translation([0, 0.8, -0.1])
    
    # Об'єднуємо
    dragon = trimesh.util.concatenate([head, horn1, horn2, center_horn])
...
def main():
    ...
    models = {
        ...
        "dragon_head.glb": create_dragon_head()
    }
